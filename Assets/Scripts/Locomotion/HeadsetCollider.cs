...
public class HeadsetCollider : MonoBehaviour {
	...
	//// Private objects ////
	private ElectricOwl armSwinger;
	...
	void Start() {
		...
		armSwinger = GameObject.FindObjectOfType<ElectricOwl>();
	...
	void OnCollisionEnter(Collision collision) {
       ...
       //     return;
		...
		if (inGeometry) {
			armSwinger.triggerRewind(ElectricOwl.PreventionReason.HEADSET);
			armSwinger.wallClipThisFrame = true;
			return;
		}

		if (armSwinger.currentPreventionReason == ElectricOwl.PreventionReason.NONE || armSwinger.currentPreventionReason == ElectricOwl.PreventionReason.HEADSET) {
